        ...
        stage('Package') {
            steps {
                withCredentials([string(credentialsId: 'machine-user-github-oauth-token', variable: 'GITHUB_OAUTH_TOKEN')]) {
                    sh '''
                        build-packer-artifact --packer-template-path "packer/build.json" --build-name "sample-app-backend" --output-properties-file "$AMI_ID_FILE"  --output-properties-file-key "$AMI_ID_KEY"
                    '''
                }
            }
        }
            ...
            steps {
                sshagent (credentials: ['machine-user-github-ssh-keys']) {
                    sh '''
                        ...
                        cd "infrastructure-live/main/$AWS_REGION/$BRANCH_NAME/services/sample-app-backend-asg"
