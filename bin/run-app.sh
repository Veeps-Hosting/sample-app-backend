#!/bin/bash
# This script is meant to be called from the User Data of a booting EC2 Instance to boot up sample-app-backend.
...
function print_usage {
  ...
  echo
  echo "This script is meant to be called from the User Data of a booting EC2 Instance to boot up sample-app-backend."
  echo
  echo "Options:"
  echo
...
# This function tries to establish a connection to the database and print the current status of migrations. If it fails to
# connect, it will retry up to 10 times, waiting 5 seconds between retries.
function wait_for_db_connection {
  local readonly db_user="$1"
  local readonly db_password="$2"
  local readonly db_connection_string="$3"
  local readonly db_migrations_location="$4"
  local i

  for (( i=0; i<=10; i++ )); do
    # The call to flyway may exit with an error if it can't reach the database. To prevent this error from causing the
    # whole script to exit, we add "&& :" at the end. For more info, see: http://stackoverflow.com/a/27793459/483528
    log_info "Checking database status at $db_connection_string"
    flyway info -user="$db_user" -password="$db_password" -url="$db_connection_string" -locations="$db_migrations_location" && :
    local readonly exit_code="$?"

    if [[ "$exit_code" -eq 0 ]]; then
      return
    else
      log_info "Failed to connect to database at $db_connection_string. Will sleep for 5 seconds and try again."
      sleep 5
    fi
  done

  log_error "Failed to connect to DB after 10 retries"
  exit 1
}

function run_migrations {
  local readonly config="$1"
  local readonly db_url="$2"
  ...
  local readonly db_connection_string="jdbc:mysql://$db_url/$db_name"
  local readonly db_migrations_location="filesystem:$SCRIPT_DIR/../sql"

  wait_for_db_connection "$db_user" "$db_password" "$db_connection_string" "$db_migrations_location"

  log_info "Applying migrations with Flyway to $db_connection_string"
  flyway migrate -user="$db_user" -password="$db_password" -url="$db_connection_string" -locations="$db_migrations_location"
}
...
function run_app {
  ...
  run_migrations "$decrypted_config" "$db_url"
